...
export function useSessionHistory(key: string | null): {
  ...
  /** Server context telemetry from the initial thread page (used by the meter). */
  contextTelemetry: { tokensEstimate: number; windowTokens: number } | null;
} {
      ...
      try {
        ...
        setState((prev) => {
          ...
          return {
            ...
            activeTurnId: typeof body?.active_turn_id === "string"
              ? body.active_turn_id
              : null,
            contextTelemetry: body?.context_telemetry
              ? {
                  tokensEstimate: body.context_telemetry.tokens_estimate,
                  windowTokens: body.context_telemetry.window_tokens,
                }
              : null,
          };
        });
        ...
        if (e instanceof ApiError && e.status === 404) {
          ...
          setState((prev) => ({
            ...
            contextTelemetry: prev.key === key ? prev.contextTelemetry : null,
          }));
        }
      }
    })();
  ...
  return {
    ...
    contextTelemetry: state.contextTelemetry,
  };
}
