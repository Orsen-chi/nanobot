...
export function ThreadShell({
  ...
  const {
    ...
    contextTelemetry: historyContextTelemetry,
  ...
  const handleTurnEnd = useCallback((info?: TurnEndContextInfo) => {
    if (chatId) activeViewportTurnByChatIdRef.current.delete(chatId);
    setSubmittedViewportTurnId(null);
    setFallbackModelName(null);
    if (info) {
      setServerContext({
        ...(typeof info.contextTokensEstimate === "number" && info.contextTokensEstimate >= 0
          ? { used: info.contextTokensEstimate }
          : {}),
        ...(typeof info.contextWindowTokens === "number" && info.contextWindowTokens > 0
  ...
  const [serverContext, setServerContext] = useState<{ used?: number; window?: number } | null>(null);
  /** Live turn_end telemetry wins; the thread-open page telemetry is the initial value. */
  const resolvedServerContext = useMemo(() => {
    if (serverContext) return serverContext;
    if (historyContextTelemetry) {
      return {
        used: historyContextTelemetry.tokensEstimate,
        window: historyContextTelemetry.windowTokens,
      };
    }
    return null;
  }, [historyContextTelemetry, serverContext]);
  useEffect(() => {
    setLocalModelPreset(null);
  }, [session?.key, sessionModelPreset]);
  // Context telemetry is per-session: only reset when the session key changes,
  // not when the model preset changes (which would drop accurate data).
  useEffect(() => {
  ...
  /** Active preset's context window (tokens). The preset wins so switching
   *  models updates the window immediately; thread telemetry and the agent
   *  default are fallbacks. */
  const activeContextWindowTokens = useMemo(() => {
    const preset = modelPresetOptions.find((option) => option.name === activeModelPreset);
    if (preset?.contextWindowTokens) return preset.contextWindowTokens;
    if (resolvedServerContext?.window) return resolvedServerContext.window;
    return settings?.agent.context_window_tokens ?? null;
  }, [activeModelPreset, modelPresetOptions, resolvedServerContext, settings]);

  /** Context tokens used by the current thread. Prefers the server's
   *  prompt-size estimate (turn_end telemetry, then the thread-open page),
   *  and falls back to a coarse CJK-aware character estimate while loading. */
  const contextTokensUsed = useMemo(() => {
    if (typeof resolvedServerContext?.used === "number" && resolvedServerContext.used >= 0) {
      return resolvedServerContext.used;
    }
    ...
    for (const message of displayMessages) {
      ...
      estimated += Math.ceil(cjk * 1.3 + other / 4);
    }
    return estimated;
  }, [displayMessages, resolvedServerContext]);
  useEffect(() => {
